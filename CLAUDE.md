...
| `core/metrics.py` | Performans metrikleri. **Birinci sınıf metrik: işlem başına ortalama R** (`PnL / açılışta riske edilen tutar`) — bileşiklenmeden bağımsız olduğu için "bu model iyi mi" sorusuna toplam getiriden daha temiz cevap verir; tablo da ona göre sıralanır. Toplam getiri, Sharpe, max drawdown, win-rate ikinci sırada raporlanır, atılmaz. **Her işlem metriği long ve short için AYRI hesaplanır ve ayrı raporlanır** (toplam değer de verilir, ama ayrışma yerine geçmez); özsermaye eğrisinden gelenler tek bakiye olduğu için hesap düzeyinde kalır. Projenin ana sorusu "short işlemler daha mı başarılı" olduğu için bu opsiyonel değil. Salt okunur — ledger'ı değiştirmez. |
...
| `trailing.atr_period` | `14` | Trailing stop'un ATR'sini hesapladığı bar sayısı. Uygulama `core/engine.py`'dedir (kural 9); periyot ortak olmalı ki aynı `trailing_atr` değeri her modelde aynı stop mesafesi anlamına gelsin. |
