...
| `scripts/probe_funding_archive.py` | **Fonlama ARŞİVİNİN ŞEMA PROBE'u**, ölçümün parçası değil: salt okunur, deftere yazmaz, `config.yaml`a dokunmaz, `data/cache/`e yazmaz, indirdiği örneği diske BIRAKMAZ (bellekte açar), hiçbir modelin davranışını değiştirmez. Cevapladığı soru `probe_funding_depth.py`nin sorusu DEĞİL, onun AÇTIĞI yoldur: A-2 geçtikten sonra (portal fonlama geçmişi sunuyor, karar 50 > DÜZELTME) *portal veri kümesinin URL ŞEMASI tutuyor mu, dosya biçimi nedir?* **Şema SIFIRDAN ARANMAZ, DOĞRULANIR:** bir gözlem olarak girdi gelir (`--url-template`) ve betik onu gerçek isteklerle sınar — kör aramadan hem hızlı hem denetlenebilir. ⚠ **ŞEMAYA UYDURMA YOKTUR:** gözlem şemayla çelişirse rapor GERÇEĞİ yazar (HTTP durumu, içerik tipi, yönlendirme hedefi) ve ikinci bir tahmin ÜRETMEZ. **Rapor YALNIZCA meta veri taşır — durum, içerik tipi, boy, arşiv üyeleri, KOLON ADLARI, damga biçimi, damga aralığı — ve hiçbir yerde bir fonlama ORANI yazmaz;** örnek satırlar maskelenir ve maskeleme **BEYAZ listedir** (`METADATA_COLUMNS`): tanınmayan kolon GİZLENİR. Ters kural (oran gibi görüneni gizle) beklenmedik adlı bir kolonu sızdırırdı ve sızacak şey tam olarak eşiğin görmemesi gereken sayıdır (docs/backtest.md > 7; test: `tests/test_probe_funding_archive.py`). Damga okumak serbesttir, gerekçesi `probe_funding_depth.py`nin aynısıdır — ızgara doğrulaması (aralık 8 saat mi) tanımı gereği damga ister. Dört soru MEKANİK olarak cevaplanır: sembol yazımı (dört varyant denenir, tahmin yok), şema eşleşmesi (`classify_schema_match`; ölçüt "DOSYA döndü mü", "200 mü" DEĞİL — 200 dönen bir HTML sayfası bir dosya değildir ve betiğin varlık sebebi bu ayrımdır), kapsam ucu (iddia edilen başlangıçtan önceki aylar da sınanır — bir kaydın ÖLÇÜMLE doğrulanması) ve örnek dosyanın yapısı. Ağ katmanı `read_sample`de biter; çözümleme `describe_payload`da SAF'tır, yani maskeleme sözü ağsız sınanır. Hiçbir istek dosya döndürmezse çıkış kodu **3** (karar 51'in "boş rapor yeşil dönmez" kuralı); belirsiz bir sonuç (istek hatası) sessizce "şema uymuyor"a DÜŞMEZ. **`PERIOD_A_START` ile arşiv kapsamı AYRI şeylerdir ve karıştırılmaz:** ilki `ema_trend`in backtest penceresidir (2022-01-01, `scripts/backtest_ema.py`) ve bu betik ona DOKUNMAZ; sınanan şey veri kümesinin kendi başlangıcıdır (iddia: 2022-03). Arşiv kapsamı pencereyi DARALTIR ama TANIMINI değiştirmez. Tetikleyicisi `.github/workflows/measure-funding.yml`in ikinci adımıdır (yalnızca `workflow_dispatch`, cron yok, varsayılan KAPALI — şema girdisi olmadan koşamaz). |
