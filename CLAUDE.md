...
| `docs/index.html` | GitHub Pages dashboard'u: harici framework/CDN yok, build adımı yok (ortak tasarım dili ve yardımcılar `docs/shared.css` + `docs/shared.js`'te). Tek veri kaynağı `docs/data/metrics.json`. **İki seviye:** (1) genel bakış — üç özet kartı (biri kapanmış İLE açık pozisyon sayısını birlikte verir: biri ölçümün örneklemi, öteki henüz sonucu olmayan pozisyonlar), LONG vs SHORT paneli (ana soru), **kıyas tablosu** (özsermaye, getiri, ort. R, kapanan, açık, MDD, rozet — hepsi tek ekranda ve `core/metrics.py`den okunur, sayfa hiçbirini hesaplamaz) ve tez tipine göre gruplanmış model kartları (aynı sayılar, KATLANMADAN; katlanan bölümde yalnızca eğri ve detay bağlantısı kalır — bir dashboard hücresinin cevabı bir tıklamanın arkasında duramaz), özsermaye eğrileri (tıklayınca izole), modeller arası getiri korelasyonu, **YAKLAŞAN KURULUMLAR** (`docs/data/proximity*.json`; ayrı bir fetch ve hiç olmayabilir — yoksa bölüm de menü girdisi de gizli kalır). Bu bölüm bir ölçüm DEĞİLDİR ve uyarısı KATLANMAZ, her katmanın üstünde TEKRAR EDER: bölüm başına tek uyarı, aşağı kaydırıp ikinci tabloyu gören okuyucuya ulaşmazdı ve tam da ulaşması gereken cümle "bu bir tahmin değil". Varsayılan görünüm yalnızca KAPIDAN GEÇEN satırlardır; kapıda ölenler katlanır (karar 34) ve ızgarada hiç tetiği olmayan semboller adıyla yazılır — boş bir liste "bilmiyoruz" ile "yok"u aynı hücreye koyardı. Kapı rozetinin rengi YÜKTEKİ karardır, sayfanın kendi karşılaştırması değil (kural 7). Katmanlar burada da ayrı kartlardadır ve tek tabloda toplanmaz; (2) model detayı — üst şerit metrikler + rozet gerekçeleri, modelin long/short kırılımı, açık pozisyonlar, sayfalı kapanmış işlem listesi (20'şer, yön filtresiyle), modelin kendi eğrisi. Detayın adresi `#model=<ad>` hash'idir: geri tuşu, yer imi ve paylaşılan link çalışır. Koyu tema, mobil öncelikli: yatay kaydırma yoktur, geniş tablolar dar ekranda kart düzenine döner, dokunma hedefleri en az 44px. Süs katmanıdır: ölçüm defterde ve JSON'dadır, sayfa yalnızca onu çizer. Scalp bölümünün tablosu base'in tablosuyla AYNI dört sütunu taşır (özsermaye, getiri, kapanan, açık): iki katmanın sayıları kıyaslanmaz (bkz. "Katmanlar") ama okuyucunun iki tabloda aynı soruyu aynı adla sorabilmesi gerekir. **Açık pozisyon sayısı sıfırken `—` DEĞİL `0` yazılır** — sayı ölçülüyor ve gerçekten sıfır; `—` "ölçülemedi" demektir (kolon tanımlarının `nan` kuralının ters yönü) ve kartla tablonun aynı olguyu iki ayrı biçimde göstermesi okuyucuya iki ayrı ölçüm gibi gelirdi. Satır satır defter görünümü bu sayfada DEĞİL, `docs/positions.html`'dedir. |
