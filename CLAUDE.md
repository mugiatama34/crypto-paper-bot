...
| `.github/workflows/measure-timesfm.yml` | `measure_timesfm.py`nin tetikleyicisi: `backtest-dc.yml`in deseni — `claude/**` dallarında İKİ tetikleyici dosyaya bağlı dar push (`.github/triggers/timesfm-parity.run` → parite, `timesfm-eval.run` → değerlendirme); aşama EKLENEN dosyadan okunur, dosya bir daha değiştirilmez. Python 3.12 + `timesfm[torch]==2.0.2` yalnızca bu işte. `permissions: contents: read`, cron YOK; sonuç log + artifact. |
