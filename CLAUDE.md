...
| `strategies/scalp_coinflip.py` | **Model 21 (`scalp` katmanının KONTROLÜ, yarışmacı):** `scalp_patient`in ikizi, tek farkı ev kapılarından GEÇMİŞ kurulumun YÖNÜNÜN adil bir yazı-turayla belirlenmesi. Katmanın `acceptance.control_model`ü budur. Ölçtüğü eksen: *kolun YÖN iddiası bilgi taşıyor mu?* **Neden `random_ctrl` değil:** base katmanının kontrolü scalp geometrisini okumaz (kendi stop mantığı, %1 tabanını ve 1.5R kapısını görmez); eklenseydi `avg_stop_distance_pct` ayrışır, ⚠B bandı yanar ve `cost_per_r` kıyaslanamaz hâle gelirdi — kontrol, kıyas ZEMİNİ olmaktan çıkıp ayrı bir maliyet ölçeğinde koşan ikinci bir model olurdu. Yazı-turada bu risk YOKTUR ve bu bir tercih değil ÖZDEŞLİKTİR: yön çevrilse de stop mesafesi aynı sayıdır. Çevirme geometriyi **YANSITIR, yeniden KURMAZ** (`flip`): stop ve hedef MESAFELERİ girişin öbür tarafına aynen taşınır, yani `stop_distance_pct` ve `reward_risk` çevirmeden ETKİLENMEZ. **Çekiliş PAYLAŞILIR** (`rng_identity` mirasla `scalp_fixed`) çünkü ölçülen eksen yön, kol/sembol seçimi değil; yazı-tura AYRI bir RNG akışından çekilir (`random_seed:as_of:scalp_coinflip:<sembol>`) — `_round_rng`in akışından çekilseydi akış ilerler ve kontrol kaynaktan BAŞKA bir kurulum seçerdi, yani eşleştirme onu kurmak için eklenen şey tarafından bozulurdu. Sembol tohuma GİRER (girmeseydi o barın tüm kurulumları aynı yüzü görür, kontrol bilgisiz değil TEK YÖNLÜ olurdu), kol GİRMEZ (girseydi çevirme kararı kol seçimine sızardı). **Kabul edilen sapma:** yansıtılan hedef yapısal engele dayanmaz — alternatif `reward_risk`i değiştirir ve S1 sağlamasını imkânsız kılardı; kontrolün bir tezi OLMAMALIDIR. Denetim izi `coin=same\|flipped` etiketidir. **Tasarımı bozulamaz** (`random_ctrl` ve `xsec_random`ın aynı sözü): buraya eklenecek her filtre kontrolü sessizce bir stratejiye çevirir. Ön-kayıt: docs/backtest.md > 6h; karar 54. |
...
| Modeller | 3 yarışmacı + 1 referans çıpası | 4 yarışmacı (12, 14, 16, 21) + 1 dış sistem kopyası (13); **21 KONTROLDÜR** (`acceptance.control_model`) | 3 yarışmacı (18 + kıyas hedefi `trend` + kontrol) + 1 çıpa |
...
| Yön iddiasının katkısı | `scalp_patient` (16) ↔ `scalp_coinflip` (21) | kurulumun YÖNÜ (kolun iddiası ↔ adil yazı-tura) | **üç kol** | **AÇIK** — katmanın C-2 kapısı budur (karar 54) |
