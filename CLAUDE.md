...
| `strategies/random_entry.py` | **Bilgisiz GİRİŞİN tek tanımı** (karar 65): kontrol modellerinin (`random_ctrl`, `trend_random`, `meanrev_random`) ortak gövdesi — uygunluk (`as_of` barı + hesaplanabilir ATR + alt sınıfın geometrisinin kurulabilirliği; bir GÖRÜŞ değil kurulabilirlik), barda TEK çekiliş (önce sembol, sonra yön), ADI SIRALI evren ve RNG kimliği `{seed}:{as_of}:{model}`. Alt sınıfın değiştirebileceği iki nokta (`_setup`, `_signal`) ÇIKIŞ geometrisidir — ölçülen eksen odur. `random_ctrl` bu gövdeye taşındı ama eski tohum biçimini (`_rng_key`, model adı YOK) korur ve sinyalleri taşımadan önceki hâliyle BİREBİR aynıdır (altın değer testi: `tests/test_random_entry.py`) — defter tarihli bölünmez (karar 25). Her kontrol KENDİ modül adıyla loglar. |
| `strategies/trend_random.py` | **Model 25 — `trend`in EŞLENMİŞ kontrolü** (base, yarışmacı; ön-kayıt docs/backtest.md > 6n): bilgisiz giriş + `trend`in çıkışı (2×ATR ilk stop + 1×ATR trailing, motorun Chandelier kuralı). Sayılar `strategies/trend.py`nin KENDİ sabitlerinden okunur. `trend`in rejim/kırılım kapısını TAŞIMAZ (taşısaydı kontrol olmazdı). Tasarımı bozulamaz. |
| `strategies/meanrev_random.py` | **Model 26 — `meanrev`in EŞLENMİŞ kontrolü** (base, yarışmacı; §6n): bilgisiz giriş + `meanrev`in 2×ATR stop'u + hedef Bollinger(20,2) orta bandına UZAKLIĞIN yön tarafına YANSITILMASI (`reflect` deseni) — orta bandın kendisi rastgele girişte yarı yarıya ters tarafta kalır ve `meanrev`in o durumdaki hedefsiz açılışı SANSÜRÜ geri getirirdi; "yalnızca bant doğru taraftayken" ise yönü ortalamaya dönüşe bağlardı. Uzaklık sıfırsa sembol o barda UYGUN DEĞİL. BTC rejim kapısı taşınmaz. Kabul edilen sapmalar (hedef mesafesi dağılımı, kural 13 sıklığı) §6n > 9'da ön-kayıtlı. |
| `tests/test_control_zero_expectation.py` | **Eşlenmiş kontrollerin DEĞİŞMEZİ** (§6n > 7 + TADİLAT-1): sürüklenmesiz aritmetik rastgele yürüyüşte (3 bağımsız 5000 barlık segment × 8 sembol, 256 alt adım) motorun KENDİSİ koşar — (a) maliyetsiz `|ort R| ≤ 0.05`, (b) maliyetle `|ort R + ort cost/R| ≤ 0.05`, (c) her pozisyon ≤ 200 barda kapanır, geçerlilik ≥ 1000 pozisyon ve SE ≤ 0.0125. Kural 13'ün "aynı barda stop+hedef → stop" varsayımı bir KÂHİNLE düzeltilir (sentetik alt adım yolu hangi seviyenin önce değildiğini bilir); motor değişmez. `random_ctrl` aynı testten KALIR (sansürün kalıcı belgesi). Sabitler ve tohum ön-kayıtlıdır, sonuç görüldükten sonra GEVŞETİLMEZ; testin fiili çözünürlüğü trailing'li geometride ≈ ±0.035R'dir (kesikli yolun stop aşımı). Her CI'da koşar (~6 dk). |
...
| `scripts/measure_market_direction.py` | **Piyasa yönü TEŞHİSİ** (docs/backtest.md > 6m, TADİLAT-1), salt okunur — TEŞHİS, KARAR DEĞİL: hiçbir model, kapı ya da karar sonuçla değişmez, sonuç yeni bir tez seçmek için kullanılmaz ve çıktı hiçbir satırda `passed`/kapı/etiket ÜRETMEZ. Cevapladığı soru: *modelin kazancının ne kadarı BTC'nin yönünden geliyor, ve model yönü rastgele girişten iyi mi seçiyor?* Her kapanmış POZİSYON için tutuş penceresinde coin ve BTC getirisi ve iki hiza tanımı — `aligned_btc` (okumada ağırlığı taşıyan) ve `aligned_coin` (neredeyse totolojik; veri görülmeden böyle yazıldı, yan yana durur). **Pencere kuralı İKİNCİ KEZ YAZILMAZ:** `core/metrics.py::_market_context`in kuralıdır (`_price_at_or_before`: kapanış ≤ açılış → kapanış ≤ kapanış) ve BTC tarafının `market_tailwind_pct` ile birebir aynı olduğu bir testle sabittir. M1 hiza payı ve grupların kazanma oranı/ort. R'si; M2 günlük OLS (β, R², α) — pozisyonsuz günler DÂHİL, gün penceresi sinyal kesiminde/son kapanışta biter (kesim sonrası harness'ındır), pozisyonda geçen gün payı yanında —; M3 haftalık tablo (işlemsiz haftalar dâhil); M4 olay kümesi (örtüşen tutuşların bağlı bileşeni), ICC, `n_etkin`, çift korelasyonu; M5 kontrol çiftlerinde hizalı pay ve β/α farkı, EŞLEŞTİRİLMİŞ hafta bootstrap'ı (`backtest_dc.py::cluster_diff_draws`). Küme = ISO hafta; < 10 hafta → aralık "değerlendirilemez" (bugün canlı defterlerin HİÇBİRİNDE hesaplanamaz). Kaynaklar yalnızca PORTFÖY koşuları: karar 59'un ema/dc defterleri `docs/data/pins/decision59/`den (SHA256 doğrulanır), xsec bir kez yeniden üretilir (`measure_regime.py::xsec_gate`), canlı defterlerin tamamı. **FİYAT KAPISI:** yeniden çekilen dolum barının açılışı, defterdeki giriş fiyatından koşunun KENDİ kayması geri çıkarılarak karşılaştırılır (göreli ≤ 1e-6); tutmayan pay %1'i aşarsa o (kaynak, dönem) ölçülmez. Bir kaynağın kapısı ötekileri rehin almaz: düşen kaynak raporda sebebiyle durur, geçenler yazılır, çıkış 3. `preflight` hiçbir R/getiri/hiza üretmez; `measure` tek seferdir. Kullanılan her fiyat ve günlük BTC serisi `docs/data/market_direction*`e sabitlenir. `strategies/*`, `core/portfolio.py`, `core/engine.py` import EDİLMEZ (test). **KOŞULDU** (#36246757531, `db87a9ca`; karar 62): geçerli kontrollü çiftlerde (dc, xsec) BTC hizalı pay farkının dört aralığı da sıfırı içeriyor, α on iki backtest satırının hepsinde sıfırı içeriyor, R² düşük (`trend` ~0 … xsec ~0.5); ema'nın farkı kontrolün sansürünü ölçtüğü için okunamaz (karar 60). Karar üretmez. |
| `.github/workflows/measure-market-direction.yml` | `measure_market_direction.py`nin tetikleyicisi: `measure-regime.yml`in deseni — `claude/**` dallarında üç tetikleyici dosya (`.github/triggers/mdir-snapshot*.run` / `mdir-preflight*.run` / `mdir-measure*.run`), aşama push aralığında EKLENEN dosyadan okunur. **Yazma yetkisi AYRI işlerdedir:** artifact/ağ okuyan işler `contents: read`; `pin` (snapshot) ve `pin-results` (ölçüm) yalnızca aynı koşunun ürettiği artifact'i sırasıyla `docs/data/pins/decision59/` ve `docs/data/market_direction*` yollarına commit eder — bu konteynerden artifact deposuna erişilemediği için sabitlemenin denetlenebilir tek yolu budur. `snapshot` karar 59 artifact'lerini (2026-10-08'de silinir) hiçbir şey hesaplamadan kopyaladı (#36243851807, `a4bab034`). Cron YOK. |
| `scripts/measure_vol_target.py` | **Oynaklık hedefleme** (docs/backtest.md > 6o, TADİLAT-1; karar 66), salt okunur ölçüm — MODEL DEĞİL: yön tahmini yoktur ve motorun boyutlandırması (kural 11) stop'suz kesirli maruziyeti ifade edemez; bu yüzden pasif bir getiri serisidir ve geçse bile hiçbir modelin boyutuna OTOMATİK girmez. İki maruziyet — BTC al-tut ve `ema` evreninin eşit ağırlıklı sepeti (uygunluk: son 30 günün kapanışı tam; `C_t` GİRMEZ) — `w = min(1, σ_hedef/σ̂)` ile ölçeklenir: σ̂ son 30 günlük log getirinin sapması × √365 (`t−1`e kadar), σ_hedef ÖNCEKİ 365 günün σ̂ medyanı. Üç strateji aynı günlerde: S (ölçeklenmiş), U (w=1), F (S'nin dönem ortalaması kadar sabit). Maliyet `devir × (fee_rate + slippage_base)`, funding HARİÇ. Sharpe/MDD `core/metrics.py::account_stats`ten, günlük kapanış `measure_regime.py::daily_closes`ten, pencereler `backtest_ema`den. Birincil ΔSR (S − U); aralık EŞLEŞTİRİLMİŞ blok bootstrap'ı, bağlayıcı olan hafta ve 4 haftalık blokların alt sınırlarının MİNİMUMU; geçme A ∧ B, varlık başına. Mekanizma (bilgi, kapı değil): σ̂_t ↔ sonraki 30 günün oynaklığı Spearman'ı, hedef penceresi dönemden taşmaz. `preflight` hiçbir getiri/σ̂/ağırlık üretmez; `measure` tek seferdir; dönem sınırları girdi değildir. `strategies/*`, `core/portfolio|engine|ledger` import EDİLMEZ (test). |
| `.github/workflows/measure-vol-target.yml` | `measure_vol_target.py`nin tetikleyicisi: `measure-regime.yml`in deseni — `claude/**` dallarında iki tetikleyici dosya (`.github/triggers/vt-preflight*.run` / `vt-measure*.run`), aşama `scripts/trigger_stage.py`den. Ölçüm işi `contents: read`; `pin-results` yalnızca aynı koşunun artifact'ini `docs/data/vol_target*`e commit eder (kullanılan günlük kapanışlar ve ağırlıklar dâhil). Cron YOK. |
| `docs/data/pins/` | Silinecek artifact'lerin SABİTLENMİŞ kopyaları (gzip + `SHA256SUMS` + `MANIFEST.json`; özet SIKIŞTIRILMAMIŞ içeriğe aittir). Bugün yalnızca `decision59/`: karar 59'un ema/dc PORTFÖY koşularının defterleri. Kopyalanırken hiçbir metrik hesaplanmadı; okuyan her araç önce SHA256'yı doğrular. Defter değildir (kural 1'in kapsamı `ledgers*/`), değişmez bir kanıt kopyasıdır. |
...
| `scripts/trigger_stage.py` | **Tetikleyici dosyadan AŞAMA çözümü** (karar 64; docs/backtest.md > 7.7), ölçümün parçası değil: salt okunur, yalnızca `git` okur. `.github/triggers/`e push ile dinleyen HER workflow (`measure-market-direction`, `measure-regime`, `measure-timesfm`, `backtest-dc`) aşamasını buradan alır ve ayrı bir `stage` işinde çözer; ölçüm işi `needs: stage` ile yalnızca aşama boş değilse başlar. **Kural:** tetikleyici, onu ekleyen merge-dışı commit push aralığındaysa VE başka hiçbir uzak dalda (ör. `origin/main`) bulunmuyorsa sayılır — tek seferlik bir aşamanın kimliği dosyanın yolu değil, onu ekleyen commit'tir. Eski kabuk kalıbı aralıkta "eklenen" dosyaya bakıyordu ve `main`i bir `claude/**` dalına birleştirmek `main`in tetikleyicilerini o dala da "eklenmiş" gösteriyordu: ölçüldü (PR #57), üç dosya birlikte geldiği için kapı reddetti; TEK dosya gelseydi `measure` gibi bir aşama başka bir dalda ikinci kez koşardı. Çıkış: tek aşama → 0; yalnızca TAŞINMIŞ tetikleyici → boş aşama, `::notice`, 0 (doğru davranış — kırmızı, her merge'de gerçek arızanın görünürlüğünü tüketirdi); hiç eklenmemiş ya da birden çok aşama → `::error`, 1. Kapı testle sabittir: `tests/test_trigger_stage.py` gerçek bir git deposu kurar ve push ile dinleyen workflow listesini `.github/workflows/`tan okuyup her birinin bu betikten geçtiğini sınar. |
...
| `acceptance.control_for` | `{}` (kök; `base`: `trend`→`trend_random`, `meanrev`→`meanrev_random`) | **Kapı** — MODEL BAŞINA kontrol (karar 65). Boşsa her yarışmacının kontrolü `control_model`dir. Doluysa TÜKETİCİDİR: statik yanlış yapılandırma (eşlenmemiş yarışmacı, kümede olmayan kontrol) `ConfigError`dır — backtest koşudan önce, canlı katmanlar testte sınanır (`core/metrics.py::check_control_map`); çalışma anında eksik kontrol (ör. kurulum hatası) yalnızca o satırın E'sini KAPATIR, tur düşmez ve koşul düşmez. Bayrak ölçüldüğü kontrolü `AcceptanceFlags.control_model`de taşır. |
...
| Modeller | 2 yarışmacı + 2 EŞLENMİŞ kontrol (`trend_random`, `meanrev_random`; `acceptance.control_for`, karar 65) + 1 referans çıpası; `random_ctrl` emekli | 4 yarışmacı (12, 14, 16, 23) + 1 dış sistem kopyası (13); kontrol **23** (`acceptance.control_model`) | 3 yarışmacı (18 + kıyas hedefi `trend` + kontrol) + 1 çıpa |
...
- **Bozuk kontrol kümede OLSA da değerlendirilemez** (`acceptance.broken_controls`, karar 63):
