...
| `.github/workflows/measure-funding.yml` | Fonlama dağılımı ölçümünün elle tetikleyicisi. `backtest-ema.yml`den ayrıdır çünkü o ön-kayıtlı bir SIRAYI (A → embargo → B) koşar ve karara giren sayıları üretir; bu iş karar üretmez ve dönem B'ye hiç dokunmaz — ikisini tek workflow'a koymak, bir dağılım sorusunun yanlışlıkla OOS penceresini de okuması demekti. Dönem sınırları workflow GİRDİSİ olarak açılmaz: açılsalardı dönem B'ye bakmak bir yazım hatası kadar kolay olurdu. `permissions: contents: read` — bu iş yazamaz. Çıktı YALNIZCA log'dur; artifact bile üretilmez. Cron YOKTUR (docs/backtest.md > 7). |
