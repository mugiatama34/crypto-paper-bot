...
| `strategies/vwap/guarded_signal.py` | **Model 18'in** sinyali: VWAP sapma-dönüşü + ÜÇ REJİM KAPISI. Çapa SEANSTIR (UTC gün), bant tabanı 2.5σ ve LONG tarafı daha geniştir (3.0σ); kurulum ancak sembolün ADX'i ve EMA eğimi trend GÖSTERMİYORSA, BTC'nin saatlik yönüne KARŞI değilse ve uçta bir tükenme izi (klimaks hacmi ya da red mumu) varsa üretilir. `signal.py` (model 14) ve `clone_signal.py` (model 13) ile kural mantığı PAYLAŞMAZ — ortak olan yalnızca `core/indicators.py` yardımcılarıdır; paylaşsalardı üç modelin farkı bir dallanmanın durumuna bağlanırdı (karar 23'ün aynı gerekçesi). Kendi kol etiketi (`vwap_revert_guard`) ve kendi `Survey`'i vardır: altı eleme sebebi tek bir "aday yok" sayısına çökerse hangi kapının modeli susturduğu ölçülemez. |
| `strategies/vwap/session_signal.py` | **F0'ın sinyali:** `clone_signal.py`nin kural kural aynısı, TEK farkla — VWAP çapası SEANSTIR (UTC gün) ve σ seansın HACİM AĞIRLIKLI σ'sudur (kaynakta kayan 300 barlık çapa + `rolling(20)` örneklem sd'si). Dönüş şartı, stop/hedef geometrisi ve eleme sebepleri birebir korunur; rejim kapısı, tükenme şartı, hedef/stop çıtası, stop tabanı ve zaman stop'u YOKTUR. Gerekçe karar 45'in dersidir: bir kapı kümesi, her biri tek başına ölçülmeden takılmaz. |
| `strategies/vwap_session.py` | **F0 (model 19):** kopyanın BİRİMİ düzeltilmiş hâli. Ayrışan tek şey çapa/σ; bant ve hedef çarpanı ÖĞRENİLMEZ, grid'in ortasında sabittir (2.0 / 0.75) — öğrenen bir model birim değişikliğinin etkisini kendi keşif payıyla karıştırırdı. Sabit teminat × 10x, kendi limitleri, üç aşamalı çıkış, bar başına kotaya kadar sinyal, evren adı sırasıyla taranır. `is_replica=True`: dış sistemin SADIK kopyası değildir (o hâlâ model 13'tür) ama bayrağın işlevsel anlamını taşır — 1R'si sabit teminattan gelir, yarışmacılarınkiyle aynı birim değildir (kural 15b). Ön-kayıt: `docs/backtest.md > 6f`; **SONUÇ: birincil tahmin P1 DÜŞTÜ** — 88 günde 647 pozisyon, ort. R −0.48, `R−market_R` −0.47 [−0.56, −0.40]. Birim kopyanın kaybını yarıya indirdi (−0.76 → −0.48, hesap −%99.91 → −%54.24, ciro yarı) ama işaretini çevirmedi; P4 de düştü (%12.4), yani "birimin katkısı" tek değişkenli bir eksen olarak okunamaz (karar 49). Katmanın `models` listesinde YOKTUR. |
| `strategies/vwap/bounce_signal.py` + `strategies/vwap_bounce.py` | **F2 / Mod B (model 21) — KOŞULMADI.** Fade DEĞİL: trend gününde (VWAP eğimi σ cinsinden yönlü) VWAP'e geri çekilmede TREND YÖNÜNDE giriş, stop VWAP'in ötesi, hedef önceki uç ya da projeksiyon. `VwapScored`tan türer, üç kanca noktasını (`_detect`, `_regime_tier`, `_extension`) ezer. Kodu durur, `REGISTRY`dedir, katmanın `models` listesinde YOKTUR ve **ön-kayıtlı bir pencerede hiç koşulmadı**: §6f'te F2, F1'in üstüne tanımlıydı ve F1 ön-kayıtlı ölüm şartı yüzünden koşulmadı (karar 49). Koşulacaksa KENDİ ön-kaydıyla gelir. |
| `strategies/vwap_inverse.py` | **Model 22 (TERS KOPYA, `is_replica=True`) — ÖLÇÜLDÜ, hipotez doğrulandı.** Kopyanın (model 13) adayını alır, girişe göre AYNALAR ve TERS yönde açar: `stop = 2E − stop`, `hedef = 2E − hedef`. Böylece stop/hedef MESAFESİ, R:R ve maliyet ölçeği birebir korunur; ayrışan tek şey yönün işaretidir (takas etmek R:R'yi de çevirirdi, o başka bir model olurdu). Kendi sinyal modülü YOKTUR (`clone_signal.py`yi okur) ve kendi config bloğu da YOKTUR — `vwap.clone.*`ı okur, çünkü burada ayrışma ölçümü bozar (genel kuralın bilinçli tersi). Cevapladığı soru: kopyanın −0.76R'si SİNYALDEN mi FRİKSİYONDAN mı? Yön çevrilince brüt işaret değiştirir, komisyon/kayma/funding ve kural 13'ün aynı-bar stop varsayımı DEĞİŞTİRMEZ; bu yüzden iki modelin ortalama R toplamı friksiyonun iki katını verir (ön-kayıtlı P4). Kol etiketi AYRI (`vwap_revert_inv`) — aynı etiket iki ZIT kural kümesini tek satırda toplar ve ortalamaları birbirini götürürdü. Çekiliş kimliği (`rng_identity`) ise kopyayla PAYLAŞILIR (model 15'in kuralı). Öğrenme mekanizması aynıdır ama kendi defterinden beslenir (kural 16), yani 13 ↔ 22 bar bar bir AYNA DEĞİLDİR. Katmanın `models` listesinde YOKTUR. Ön-kayıt: `docs/backtest.md > 6g`; **SONUÇ:** 88 günde n=5.652, ort. R −0.74 ↔ kopyanın −0.76'sı; toplam −1.50 ⇒ **F = 0.75R/pozisyon, G = −0.01R.** Yani kurulumun brüt beklentisi SIFIR ve kaybın tamamı friksiyon; ters çevirmek kurtarmıyor çünkü kurtarılacak işaret yok (karar 50). Katmanın `models` listesine GİRMEZ. |
| `strategies/vwap_scored.py` | **F1 (model 20) — KOŞULMADI.** F0'ın sinyali + BEŞ kalemlik maliyet/çıkış demeti: skorla SIRALAMA ve BOYUT (`size_scale`, yalnızca küçültür), post-only limit dolum (bir bar geçerli, dolmazsa `limit_not_filled`), ilerleme koşullu zaman stop'u, 5x, ve P&L'ye değil 24s hacme bakan likidite kuralı. Karar 49: F0'ın ham ortalama R'si negatif çıktığı için ön-kayıtlı ölüm şartı işledi ve F1 koşulmadı — maliyet tarafı 0.48R'lik açığı kapatacak büyüklükte değil (maker'a geçiş tur maliyetinin ~%0.035'i). Kodu ve `vwap.scored.*` config bloğu durur (kural 1'in aynı gerekçesi: kapanan eksen satırı silmez). |
| `strategies/vwap_guarded.py` | **Model 18:** kopyanın (model 13) CANLIYA HAZIRLANMIŞ uyarlaması — kopyanın ÜSTÜNE YAZILMAZ (kural 15b: değiştirilen kopya kopya olmaktan çıkar), ayrı bir modeldir. Sekiz kalemde ayrışır: seans çapası, 2.5/3.0σ bant, rejim kapıları, tükenme şartı, risk boyutlandırma (kural 11; sabit teminat YOK), 10 barlık zaman stop'u, kendi evreni (PENGU/ETHFI yok) ve risk kesicileri. **Kesiciler R cinsindendir** çünkü model bakiyeyi göremez (kural 7): günlük −2R zarar limiti ve `max_drawdown_pct / risk_per_trade` (= 8R) drawdown kill-switch'i; ikisi de yalnızca YENİ KURULUM ARAMAYI durdurur, açık pozisyonların yönetimi motorda sürer. **Parametre öğrenimi kopyanın iskeletini korur ama üç yerde ayrışır:** ödül DRAWDOWN ile cezalandırılır (`skor = (Σr − w × maxDD) / n`; maliyet zaten R'nin payındadır, çünkü `pnl` nettir), sembol eşiği 3 değil 30'dur (`acceptance.min_trades` ile aynı sayı) ve ısınma GLOBALDİR — sembol başına ısınma 9 kol × 11 sembol ile tüm örneklemi saf keşfe harcardı. Keşif payı 0.25 değil 0.10, ama sıfır DEĞİL (susturulan kol bir daha ölçülemez). Stop çarpanı grid'in ekseni DEĞİLDİR: stop mesafesi maliyet ölçeğidir (kural 14) ve onu öğrenmeye açmak modelin kendi ⚠B bandını koşu sırasında kaydırması olurdu. **Korelasyon kotası** (aynı yönde en çok 2 pozisyon) modelin KENDİ açık pozisyonlarını sayar ve bu sayıyı sözleşmenin verdiği yerden (`manage_positions`) okur — kural 16'nın yasağı bir SONUCU öğrenmeye sokmak üzerinedir, bir SAYIYI saymak üzerine değil. Görüntü bir bar bayattır, bu yüzden o barın kendi sinyali de kotaya sayılır. Barda tek sinyal. 13 ↔ 18 bir EKSEN DEĞİL, bir toplam farktır. |
...
| `vwap.session.*` | model 19 (F0) | Kopyanın BİRİMİ düzeltilmiş hâlinin sayıları: `band_mult` 2.0 ve `tp_mult` 0.75 (kaynağın grid'inin ORTASI, süpürülmedi), `sl_mult` 0.5 (kaynakta zaten sabit), `min_bars` 8 (seans barı; model 14'ün `min_vwap_bars` değeriyle aynı sayı), `notional_fraction` 0.5 × `leverage` 10 ve kendi limitleri. Boyut/maker dolum/skor/zaman stop'u BURAYA DEĞİL F1'e aittir. |
| `vwap.guarded.*` | model 18 | Uyarlamanın KENDİ sayıları; `vwap.band_mult` ve `clone.*` ile PAYLAŞILMAZ: bant (`band.short` 2.5 / `band.long` 3.0), stop (`atr_multiple` 2.5 — model 14 ile aynı ölçek), `time_stop_bars` (10), rejim kapısı (`regime.adx_max` 22, `regime.max_slope_atr` 1.5), tükenme (`exhaustion.*`), BTC yönü (`btc_bias.*`, 1s), öğrenme (`bandit.*`: 3 bant × 3 hedef oranı, `epsilon` 0.10, `min_symbol_samples` 30, `drawdown_weight` 1.0, `long_extra` 0.5) ve risk kesicileri (`risk.daily_loss_r` 2.0, `risk.max_drawdown_pct` 0.08, `risk.max_correlated_positions` 2). Hiçbiri süpürülmedi; ön-kayıt `docs/backtest.md > 6d`. |
...
| Modeller | 10 yarışmacı + 1 referans çıpası | 3 yarışmacı (12, 14, 16) + 1 dış sistem kopyası (13) |
...
| Üç kalemlik fark ⚠ | `vwap_clone` (13) ↔ `vwap_session` (19, F0) | **tek değişken DEĞİL** — çapa/σ + parametre öğrenimi (kopya öğrenir, F0 sabit) + evren (13 ↔ 12, fark SUI) | KAPALI — F0 ölçüldü (n=647): birim kaybı YARIYA indirdi (−0.76 → −0.48) ama işareti çevirmedi; `R−market_R` −0.47 [−0.56, −0.40]. Ön-kayıtlı ölüm şartı işledi, F1/F2 koşulmadı (karar 49) |
| **Yönün işareti** | `vwap_clone` (13) ↔ `vwap_inverse` (22) | yalnızca yön (aynı kurulum, aynalanmış seviyeler, aynı R:R) | KAPALI — ölçüldü: −0.76 ↔ −0.74, toplam −1.50 ⇒ **friksiyon 0.75R/pozisyon, brüt beklenti −0.01R.** Kurulum hiçbir yönde bilgi taşımıyor; VWAP fade hattı kapandı (karar 50) |
| Sekiz kalemlik toplam fark ⚠ | `vwap_clone` (13) ↔ `vwap_guarded` (18) | **tek değişken DEĞİL** | KAPALI — 54 günde 8 kurulum, ölçülemedi; 18 kâğıttan çıkarıldı (karar 45/46) |
...
kapılardan geçirir. VWAP modelleri (13, 14) ise **ayrı sinyal modülü** okur — model 14
...
| `excess_r` + `excess_r_ci_low/high` | `mean(R_i − market_R_i)`, POZİSYON BAZINDA eşleştirilmiş, ve onun bootstrap aralığı. `avg_r − market_r` ile aynı şey DEĞİLDİR: o iki ortalamanın farkıdır ve iki ortalama farklı satır kümelerinden gelir (R her pozisyonda vardır, `market_R` yalnızca fiyatlananlarda) — ölçülemeyen bir pozisyonun R'si farkın içine "piyasa sıfır verdi" varsayımıyla sızardı. Aralık ancak eşleştirilmiş farkla kurulabilir. Bir ÖLÇÜ, bir KAPI değil; `docs/backtest.md > 6f`in birincil ölçüsüdür. |
