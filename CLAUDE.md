...
| `strategies/base.py` | Tüm stratejilerin uyacağı soyut arayüz (`Strategy`, `Signal`, `MarketData`). Mantık içermez, yalnızca sözleşme. |
...
## Strateji Arayüz Sözleşmesi (taslak — henüz kod olarak uygulanmadı)

```python
class Strategy(ABC):
    name: str
    allowed_directions: list[str]  # ["long"], ["short"] veya ["long", "short"]
...
class Signal:
    ...
    entry_type: str           # "market" | "limit" vb.
    stop_price: float
...
Bu sözleşme onay bekliyor. `strategies/base.py` şu an yalnızca bu tasarıma referans veren bir
taslak içeriyor; gerçek `ABC`/`dataclass` kodu onay sonrası yazılacak.
