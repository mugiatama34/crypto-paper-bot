...
| `strategies/dc/signal.py` | **Ölüm kesişimi + geri çekilme kurulumunun TEK tanımı** (modeller 22-23 aynı kopyayı okur; ön-kayıt docs/backtest.md > 6i). Tanımlar ölçülebilirlik sayımıyla (`scripts/measure_death_cross.py`) BİREBİR aynıdır: kesişim (c'de EMA50 < EMA200, c−1'de ≥), rejim (kesişimden sonra EMA50 < EMA200 sürdükçe), kurulum barı (`high ≥ EMA50`, `close < EMA50`, `close < open`). Geometri de burada: stop kurulum barındaki EMA200, hedef `min(low[c .. t−1])` — **kurulum barı HARİÇ** (dâhil edilseydi "hedef zaten geçilmiş" kuralı fiilen boş kalırdı); `t = c` ise `target_undefined`, `close ≤ hedef` ise `target_passed` (kural 8'in zorunluluğu: short hedefi kapanışın altında olmalı). **Görüş penceresi bir MODEL kuralıdır** (`dc.lookback_bars` 3000, §6i > TADİLAT-1): canlıda modele `data.history_bars` bar verilir, backtest'te motor her bara yüklenen verinin TAMAMINI verir (`core/engine.py::_snapshot`) — kesişimin görülebilirliği pencereye bağlı olduğu için pencere iki ortamda aynı olacak biçimde modelde kesilir. Pencerenin ilk `warmup_bars` (600) barı EMA200 ısınmasıdır; kesişim ondan SONRA aranır, bulunamazsa `cross_not_visible`. **Stop tavanı burada YOKTUR** — motorundur. Sayım kodlarının sırası ön-kayıtlıdır ve her sembol tam bir koda düşer. `reflect` kontrolün aynalamasının tek kopyasıdır (kurulum kapanışı etrafında, mesafeler korunur). |
...
| `strategies/dc_coinflip.py` | **Model 23 (`dc` katmanının KONTROLÜ, yarışmacı):** `dc_short`tan TÜRER, ayrışan TEK şey yöndür — yazı-tura "ters" gelirse stop ve hedef mesafeleri kurulum kapanışı etrafında aynalanır (`reflect`), yani tavan kapısı iki modelde birebir aynı çalışır. RNG ayrı akış ve SEMBOL bazında çatallı (`random_seed:as_of:dc_coinflip:sembol`); tohum tek seferlik (§6i > 4). Denetim izi `coin=same|flipped` (S2 buradan okunur). `wave_coinflip`in deseni. Katmanın `acceptance.control_model`ü budur. |
...
| `dc.*` | modeller 22-23 | `fast_period` (50), `slow_period` (200), `warmup_bars` (600 — ölçülebilirlik sayımının `WARMUP_BARS`ıyla aynı, test sınar), `lookback_bars` (3000 — modelin GÖRÜŞ penceresi, canlıda ve backtest'te aynı; §6i > TADİLAT-1). Yalnızca `dc_short` ve `dc_coinflip` okur; dördü de docs/backtest.md > 6i'de ÖN-KAYITLIDIR. ATR burada YOKTUR: model ATR kullanmaz (stop EMA200, hedef önceki dip), ATR yalnızca motorun tavan kapısındadır. |
